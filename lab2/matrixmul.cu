...
void MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
    ...
    //int division will lop off one, so maybe need to add 1 if M.width % TILE_WIDTH != 0
    int GRID_WIDTH = N.width / TILE_WIDTH;
    if ( N.width % TILE_WIDTH != 0 ) {
        GRID_WIDTH++;
    }
   
    int GRID_HEIGHT = M.height / TILE_HEIGHT;
    if ( M.height % TILE_WIDTH != 0 ) {
        GRID_HEIGHT++;
    }
    ...
    MatrixMulKernel<<<BlocksPerGrid, ThreadsPerBlock>>>(Md, Nd, Pd); 
