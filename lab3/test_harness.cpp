...
int main(int argc, char* argv[])
{
    ...
    uint32_t *d_input;
    uint8_t *d_bins;
    opt_2dhisto_setup(d_input, input, d_bins, kernel_bins); 
    /* End of setup code */

    /* This is the call you will use to time your parallel implementation */
    TIME_IT("opt_2dhisto",
            1000,
            opt_2dhisto(d_input, d_bins);)

    /* Include your teardown code below (temporary variables, function calls, etc.) */
    printf("Teardown code\n");
    opt_2dhisto_teardown(d_input, d_bins, kernel_bins);
    printf("End of teardown code\n");
    ...
    printf("Printing gold_bins\n");
    for (int i=0; i < HISTO_HEIGHT*HISTO_WIDTH; i++) {
        printf("%d ", gold_bins[i]);
    }
    printf("\n");
    printf("Printing kernel_bins\n");
    for (int i=0; i < HISTO_HEIGHT*HISTO_WIDTH; i++) {
