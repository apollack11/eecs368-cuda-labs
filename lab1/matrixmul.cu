...
int main(int argc, char** argv) {
	...
	/* USED FOR DEBUGGING
	// Print CPU result
	printf("MATRIX FROM CPU CODE \n");
	for (int q = 0; q < reference.width * reference.height; q++) {
		printf("%.2f ", reference.elements[q]);
		if (q % 16 == 15) printf("\n");
	}	
	// Print GPU result
	printf("\n\nMATRIX FROM GPU CODE \n");
	for (int q = 0; q < P.width * P.height; q++) {
		printf("%.2f ", P.elements[q]);
		if (q % 16 == 15) printf("\n");
	}	
	*/
...
void MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
	...
	Matrix N_d;
	Matrix P_d;
 	M_d = AllocateDeviceMatrix(M);
	N_d = AllocateDeviceMatrix(N);
	P_d = AllocateDeviceMatrix(P);

	// copy matrices to device
	CopyToDeviceMatrix(M_d, M);
	CopyToDeviceMatrix(N_d, N);
	CopyToDeviceMatrix(P_d, P);

	// setup the execution configuration
	dim3 dimGrid(1, 1);
	dim3 dimBlock(M.height, M.width);	
	
	// launch the device computation threads
	MatrixMulKernel<<<dimGrid, dimBlock>>>(M_d, N_d, P_d);

	// copy result matrix back to device
